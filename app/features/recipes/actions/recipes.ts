...
export async function generateRecipe({ url, id }: { url: string, id: string }) {
    return fetch(`https://smuk7f1osf.execute-api.us-east-1.amazonaws.com/generate-recipe?url=${url}&id=${id}`)
        .then(response => {
            if (!response.ok) {
...
export async function updateRecipe(id: string, data: Partial<InsertRecipe>) {
    const [updatedRecipe] = await db.update(recipesTable).set(data).where(eq(recipesTable.id, id)).returning()
